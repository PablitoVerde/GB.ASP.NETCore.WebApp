...
namespace Lesson3.Repository
{
    ...
    {
        static public List<Person> data;
...
new Person { Id = 50, FirstName = "Ramona", LastName = "Gilliam", Email ="massa.Vestibulum@lectuspede.ca", Company = "Imperdiet Dictum LLP", Age = 24 },
        ...
        public List<Person> GetByName(string SearchName)
        {
            List<Person> list = new List<Person>();
            return list = data.Where(x => x.FirstName == SearchName || x.LastName == SearchName).ToList();
        }

        public List<Person> GetAllPagination(int skip, int take)
        {
            List<Person> list = new List<Person>();
            return list = data.Skip(skip).Take(take).ToList();
        }
        ...
        public async Task Update(Person item)
        {
            await Task.Run(() =>
            {
                if (GetById(item.Id) is Person person)
                {
